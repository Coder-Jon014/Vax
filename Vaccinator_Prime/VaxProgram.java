...
public class VaxProgram { 
    ...
    public void loadApproved(String afile)
    {
        ...
        try {
           ...
           // System.out.println("File not found in current directory " + currentDir);
        }
    }
    public ArrayList<VaccineBatch> loadVCBatches (String vcfile) throws IOException, ArrayIndexOutOfBoundsException {
        ...
        while (vscan.hasNextLine()) {
            ...
            if (nextLine.length > 3) {
                contraIndicators = nextLine[3];
            }

            VaccineBatch vac = new VaccineBatch(name, size, preference, contraIndicators);
            vlist.add(vac);
        }
    ...
    {
        int pos = -1;
        for (BasePerson person: p) {
            if (person.getId() == id){
                return p.indexOf(person);
            }
        }
        return  pos;
